...
_const.COMMANDS.forEach((_ref2) => {
  ...
  _commander.program.command("".concat(mode, " <file...>")).description("".concat(description)).requiredOption(password.label, password.description).option(cipher.label, cipher.description, cipher.defaultValue).option(encoding.label, encoding.description, encoding.defaultValue).option(silent.label, silent.description, silent.defaultValue).action((command, options) => {
    try {
