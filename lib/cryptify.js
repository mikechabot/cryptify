...
function _getOptionsFromCLI(searchIn, searchFor, getArgument) {
    return searchIn.map(function (value, index) {
...
function CryptifyConfig(configArguments) {
    ...
    var commands = _getOptionsFromCLI(configArguments, CONST.REQUIRED_COMMANDS);
    var passwords = _getOptionsFromCLI(configArguments, CONST.OPTIONS.PASSWORD, true);
    var ciphers = _getOptionsFromCLI(configArguments, CONST.OPTIONS.CIPHER, true);

    // Parse file names from CLI
    configArguments.forEach(function (value, index) {
        if (!_includes(CONST.REQUIRED_COMMANDS, value) && !_includes(CONST.OPTIONS.PASSWORD, value) && !_includes(CONST.OPTIONS.CIPHER, value) && !_includes(CONST.OPTIONS_DO_NOT_TAKE_ARGUMENT, value) && !_includes(CONST.OPTIONS_TAKE_ARGUMENT, configArguments[index - 1])) {
...
CryptifyConfig.prototype.isVerbose = function () {
    return this.command === CONST.OPTIONS.LOG[0] || this.command === CONST.OPTIONS.LOG[1];
};
...
function _cryptify(options) {
    options.doEncrypt() ? _encrypt(options, options.getCipher() || DEFAULT_CIPHER) : _decrypt(options, options.getCipher() || DEFAULT_CIPHER);
}
...
function _encrypt(options, cipherAlgorithm) {
    options.getFiles().forEach(function (file) {
        ...
        var cipher = crypto.createCipher(cipherAlgorithm, options.getPassword());
...
function _decrypt(options, cipherAlgorithm) {
    options.getFiles().forEach(function (file) {
        ...
        var outputPath = path.join(file + '.' + TEMP);
        // Generate cipher and open streams
        var cipher = crypto.createDecipher(cipherAlgorithm, options.getPassword());
        var is = fs.createReadStream(inputPath);
        var os = fs.createWriteStream(outputPath);
        // Encrypt files
        is.pipe(cipher).pipe(os);
