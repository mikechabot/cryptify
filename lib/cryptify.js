...
CryptifyConfig.prototype.__init = function (configOptions) {
    ...
    if (this.doReturnFiles()) {
        ...
        var encoding = _parseOptions(configOptions, _const.OPTION.FILE_ENCODING, true);
...
CryptifyConfig.prototype.getExtension = function () {
    return this.doEncrypt() ? _const.EXTENSIONS.ENCRYPT : _const.EXTENSIONS.DECRYPT;
};
...
CryptifyConfig.prototype.doKeepFiles = function () {
    return _contains(this.options, _const.OPTION.KEEP);
};
            ...
            _cryptify(config).then(function (closeEventCount) {
                ...
                if (!config.doReturnFiles() || config.doEncrypt()) {
                    ...
                    config.getFiles().forEach(function (file) {
                        fileContents.push(_fs2.default.readFileSync(file, config.getEncoding()));
                    });
...
function _cryptify(options) {
    return new Promise(function (resolve) {
        ...
        options.getFiles().forEach(function (file) {
            ...
            var outputPath = _path2.default.join(file + '.' + options.getExtension());

            options.log('Acting on ' + inputPath);

            options.log('Getting cipher');
            var cipher = options.getCipherFunction()(options.getCipher(), options.getPassword());
