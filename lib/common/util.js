    ...
    args.forEach(function (value, index) {
        if (!_const.OPTION_ARRAY.includes(value) && !_const.OPTIONS_WITH_ARGS.includes(args[index - 1])) {
            if (!_fs2.default.existsSync(value)) {
                throw new _exception2.default('No such file: "' + value + '"');
 ...
 * @private
 */
function deepFind(lookIn, lookFor) {
...
function isValidPassword(password) {
    ...
    return password !== null && password !== undefined && typeof password === 'string' && password.trim().length >= 8 && deepFind(password.trim().split(''), _const.SPECIAL_CHARACTERS) && _hasNumber(password) && _hasUpperCase(password) && _hasLowerCase(password);
}
