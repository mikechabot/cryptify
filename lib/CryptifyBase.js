...
class CryptifyBase {
  ...
  getKey() {
    var digestHash = _crypto.default.createHash(_const.DEFAULT.HASH_ALGORITHM).update(this.password).digest();

    var matches = this.algorithm.match(keySizeRegex);

    if (matches) {
      var keySize = parseInt(matches[0]);
      var length = (0, _funcs.getBufferLength)(keySize);
